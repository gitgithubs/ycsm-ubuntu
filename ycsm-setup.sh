...
ycsm_install() {
  ...
  apt-get install -y vim less sudo nano software-properties-common dirmngr apt-transport-https lsb-release ca-certificates python3-software-properties python-software-properties
...
ycsm_initialize() {
  ...
  certbot --nginx --register-unsafely-without-email --non-interactive --quiet --register-unsafely-without-email --agree-tos -d $domain_name;
  check_errors
